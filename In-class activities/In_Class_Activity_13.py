...
def main():
    ...
    if user_input == 'p':
        ...
        if 0 <= page_number <= 5508:
            page_links = generate_links(main_link, page_number)
            selected_page_link = page_links[0]  # Use the first link
        ...
        if 0 <= page_number <= 5508:
            links = generate_links(main_link, page_number)
            selected_page_link = links[0]  # Use the first link
...
def generate_links(start_link, page_number):
    """
        This method generates a link for a specific page
        :param start_link: The start link of the bug tracking system.
        :param page_number: The page number to view (0-5508)
        :return: The link to the specific page
    """
    page_link = f"{start_link}&start={page_number * 75}"
    return [page_link]  # Return as a list to match the previous structure
